...
implementation
//##############################################################################
function TPOISK2.RestartLoad(Stream: TStream;Count: integer;const TimeShift:double):boolean;
 var j: integer;
begin
  Result:=True;
...
//-------------------------------------------------------------------------------------------------
procedure   TPOISK2.RestartSave(Stream: TStream);
 var j: integer;
begin
...
//-------------------------------------------------------------------------------------------------
PROCEDURE TPOISK2.StepOneParam;

  label 3, 6, 8, 10, 12;

  // Метки состояния
  Label lbl_1, lbl_2, lbl_3, lbl_4, lbl_5;
...
//##############################################################################
  lbl_4:
    ...
    OUT2(@XN[0], @FXN[0], N, M, IterationNum, otp_step_position);
    Inc(IterationNum);

    // Найден оптимум !!!
    if (FXN[M] <= 0.001) then begin
      X^[0] := XN[0];
      FX^[M] := FXN[M];
      StopOpt := 1;
      goto lbl_exit
...
//-------------------------------------------------------------------------------------------------
PROCEDURE TPOISK2.StepFewParam;
